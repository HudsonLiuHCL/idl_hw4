...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        # Filter for .npy files and sort them
        ...
        for file in tqdm(self.text_files):
            ...
            transcript_array = np.load(file)
            # Convert array to list of strings and join
            transcript = ''.join(transcript_array.tolist())
            ...
            # Shifted: [SOS, token1, token2, ..., tokenN]
            # Golden:  [token1, token2, ..., tokenN, EOS]
        ...
        print(f"Loaded {self.length} transcripts")
        print(f"Total characters: {self.total_chars:,}")
        print(f"Total tokens: {self.total_tokens:,}")
        print(f"Average chars per token: {self.avg_chars_per_token:.2f}")
        print(f"Max sequence length: {self.text_max_len}")
    ...
    def collate_fn(self, batch: List[Tuple[torch.LongTensor, torch.LongTensor]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Lengths should be the actual sequence length (same for shifted and golden)
        ...
        # pad_sequence expects (seq_len, batch) by default, so we need batch_first=True
