...
class MultiHeadAttention:
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        if key_padding_mask is not None:
            ...
            # Broadcast will happen automatically during logical_or
    ...
    def _split_heads(self, x):
        ...
        N = x.shape[0]
        seq_len = x.shape[1]  # Could be L or S depending on input
        d_k = self.embed_dim // self.num_heads
        
        # Reshape: (N, L, embed_dim) -> (N, L, num_heads, embed_dim // num_heads)
        x = x.reshape(N, seq_len, self.num_heads, d_k)
    ...
    def _concat_heads(self, x):
        ...
        N = x.shape[0]
        seq_len = x.shape[2]  # Could be L or S depending on input
        ...
        x = x.reshape(N, seq_len, self.embed_dim)
